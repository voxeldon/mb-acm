...
class AddonConfigurationManager {
    ...
    print_load_list(addonData) {
        let rawMessage = RawText.MESSAGE(RawText.TEXT("§l§6A.C.M§r\n"));
        addonData.forEach(addon => {
            const packName = addon.meta.pack_name;
            rawMessage = RawText.MESSAGE(rawMessage, RawText.TEXT("§2Loaded:§r "), RawText.TRANSLATE(`acm.${addon.meta.team_name}.${packName}`), RawText.TEXT("\n\n"));
        });
        ...
        system.runInterval(() => {
            ...
            ALL_PLAYERS.forEach(player => {
                if (player.isSneaking)
                    this.handle_form(player, addonData);
            });
        }, 0);
    }
    get_world_data() {
