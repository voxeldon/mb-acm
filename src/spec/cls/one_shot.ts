...
*/
class OneShot {
    private ONESHOT_TRACKER: Map<string, Record<string, boolean>>;

    constructor() {
        this.ONESHOT_TRACKER = new Map<string, Record<string, boolean>>();
    }
     ...
     */
    handleOneShot(id: string, condition: string, state: boolean, func: () => void): void {
     ...
     */
    deleteOneShot(oneShotId: string): void {
        if (!this.ONESHOT_TRACKER.has(oneShotId)) return;
